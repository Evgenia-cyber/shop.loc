...
class Image extends \app\models\AppModel {

public static function uploadImg($name, $wmax, $hmax) {
        $uploaddir = WWW . '/images/';
        $ext = strtolower(preg_replace("#.+\.([a-z]+)$#i", "$1", $_FILES[$name]['name'])); // расширение изображения
        ...
        if (@move_uploaded_file($_FILES[$name]['tmp_name'], $uploadfile)) {
            if ($name == 'news-img') {
                $_SESSION['news-img'] = $new_name;
            } elseif ($name == 'slide-img') {
                $_SESSION['slide-img'] = $new_name;
            } elseif ($name == 'single') {
