...
class ProductController extends AppController {
    ...
    public function addAction() {
        if (!empty($_POST)) {
            ...
            if ($id = $product->save('product')) {
                ...
                if ($data['modification_title'][0] !="") {
                 $product->saveModification($id, $data);
                }
    ...
    public function deleteSingleImgAction() {
        $id = isset($_POST['id']) ? $_POST['id'] : null;
        $src = isset($_POST['src']) ? $_POST['src'] : null;
        if (!$id || !$src) {
            return;
        }
        if (\R::exec("UPDATE product SET img='no_image.jpg' WHERE id=?", [$id])) {
            @unlink(WWW . "/images/$src");
            exit('1');
        }
        return;
